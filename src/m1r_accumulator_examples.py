...
#
#              for k in range(r - s): There is supposed to be a comma between the r and s
#                  count = 0 This is supposed to be located above the k loop
#                  if math.cos(r) > 0: 'r' is supposed to be 'k' in this case
#                      count = 1 This should be count = count+1
#                  return count  This also should be even with the for k in range line
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean? The underscore means that it is not used as a variable, but the function runs the same.
#
...
#                  circle = rg.Circle(point, radius)
#                  circle.attach_to(window)
#                  x = x + (2 * radius) This is the wrong one because the x value is adjusted twice when it should be only adjusted once.
#
###############################################################################
