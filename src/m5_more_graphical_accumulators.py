...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
            square = rg.Square(rg.Point(x1+k*radius,y1+k*radius),radius*2)
...
def run_test_draw_circles_from_rectangle():
    ...
    title = title + ' 7 little circles from green square, 4 big circles'
    ...
    title += ' 20 teeny circles from blue square!'
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    xcent = rectangle.get_center().x
    ycent = rectangle.get_center().y
    y1 = rectangle.corner_1.y
    rectangle.attach_to(window)


    r = rectangle.get_height()/2
    x = rectangle.get_center().x-(rectangle.get_height()/2+rectangle.get_width()/2)
    y = rectangle.get_center().y
    for k in range(m):
        circle = rg.Circle(rg.Point(x-2*k*rectangle.get_height()/2,y),r)
        circle.fill_color = rectangle.fill_color
        circle.attach_to(window)
    window.render()
    for k in range(n):
        circle = rg.Circle(rg.Point(rectangle.get_center().x , rectangle.get_center().y-((rectangle.get_width()/2+rectangle.get_height()/2)+2*k*rectangle.get_width()/2)) , rectangle.get_width()/2)
    ...
    #for k in range(m+1):
    #   if k == 0:
            #x = xcent - (math.fabs( -ycent + y1) - math.fabs( -xcent + x1))
            #r = math.fabs(ycent - y1)

    #circle = rg.Circle(rg.Point(rectangle.get_center().x-(rectangle.get_width()/2+rectangle.get_height()/2), ycent), rectangle.get_center().y/2)
    #circle.attach_to(window)
        #else:
    #        circle = rg.Circle(rg.Point((x1+2*k*math.fabs(ycent-y1)),ycent),ycent-y1)
    #        circle.attach_to(window)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    x1 = rectangle1.get_width()/2
    y1 = rectangle1.get_height()/2
    center2x = rectangle2.get_center().x
    center2y = rectangle2.get_center().y
    #x2 = rectangle2.get_lower_left_corner().x
    #y2 = rectangle2.get_lower_left_corner().y
